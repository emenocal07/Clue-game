...
//Gets random item from each array and construct the mistery
  ...
  let mystery = solveMystery() // Who killed Mr. Buddy with what in which room
 
  showArrays(suspectsArray)
  showArrays(weaponsArray)
  showArrays(roomsArray)

// loops throught all of our arrays and shows eachItem's name
...
function guess() {
    ...
    if (guessedSuspect === mystery.suspect.name 
        ...
        alert(mystery.suspect.name + " killed Mr. Buddy in the " + mystery.room.name + " with the " + mystery.weapon.name)
    }
